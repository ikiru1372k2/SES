...
export function validateWorkbookDescriptor(file: { name: string; size: number }): void {
...
export function validateWorkbookFile(file: File): void {
  validateWorkbookDescriptor(file);
}

export async function parseWorkbookBuffer(buffer: ArrayBuffer | Uint8Array, fileName: string): Promise<WorkbookFile> {
  validateWorkbookDescriptor({
    name: fileName,
    size: buffer instanceof Uint8Array ? buffer.byteLength : buffer.byteLength,
  });
...
export async function buildAuditedWorkbookBuffer(file: WorkbookFile, result: AuditResult, corrections: Record<string, IssueCorrection> = {}): Promise<ArrayBuffer> {
  const ExcelJS = (await import('exceljs')).default;
  const workbook = new ExcelJS.Workbook();
...
export async function downloadAuditedWorkbook(file: WorkbookFile, result: AuditResult, corrections: Record<string, IssueCorrection> = {}): Promise<void> {
  ...
  const includeCorrections = Object.keys(corrections).length > 0;
  const buffer = await buildAuditedWorkbookBuffer(file, result, corrections);
